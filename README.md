...
* **The secret-leak test looks for sequences, not single words.** The receipt's secret-leak test generates a fresh BIP-39 seed on every run and sweeps `recibo.json`, `recibo.md` and the inspected object for it. It first searched word by word, and that was noise rather than detection: dozens of BIP-39 words are ordinary English that a payment receipt writes with every right — a run drew `dry` and the test reported a leak that did not exist, against the `dry` inside `dry-run`. What is conclusive is the sequence, so it now asserts that neither the full phrase nor any window of three consecutive seed words appears, plus 32-byte hex key material. Ten consecutive runs, each with a different random seed, are green.
